...
while true; do
    if ! rclone ls $RCLONE_REMOTE:$LOCK >/dev/null 2>&1; then
        # Lock doesn't exist, create it
        echo "$OWNER $(date +%s)" | rclone rcat $RCLONE_REMOTE:$LOCK
        echo "✅ Lock acquired"
        break
    else
        # Lock exists, check if stale
        LOCK_TIME=$(rclone cat $RCLONE_REMOTE:$LOCK | awk '{print $2}')
        NOW=$(date +%s)
        AGE=$((NOW - LOCK_TIME))
        if (( AGE > STALE )); then
            ...
            LOCK_OWNER=$(rclone cat $RCLONE_REMOTE:$LOCK | awk '{print $1}')
